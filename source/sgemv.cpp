                                              ...
                                              float *Y,const long yOffset, const int incY)
{
    ...
    Concurrency::array_view<float> yView(lenYn, Y);  
   	if(order)
        	...
        	gemv_AMP_rMajor(accl_view, type, M, N, *alpha, aMat, aOffset, xView, xOffset, incX, *beta, yView, yOffset, incY, tempBuf);
    }
...
{ 
  ...
  if( alpha == 0 && beta == 1)
    ...
    if(order){
        ...
        gemv_AMP_rMajor(accl_view, type, M, N, alpha, A, aOffset, X, xOffset, incX, beta, Y, yOffset, incY, tempBuf);
    }
  ...
  if( alpha == 0 && beta == 1)
    ...
    Concurrency::array_view<float> tempBuf(num_blocks * lenYt, temp);
    	...
    	if(order)
        	...
        	gemv_AMP_rMajor(accl_view, type, M, N, alpha, A, aOffset, A_batchOffset, X, xOffset, X_batchOffset, incX, beta, Y, yOffset, Y_batchOffset, incY, tempBuf, batchSize);
    return AMPBLAS_SUCCESS;
}
