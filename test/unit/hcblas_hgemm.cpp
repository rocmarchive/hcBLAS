      ...
      for( int i = 0 ; i < M ; i++)
      {
         for( int j = 0 ; j < N ; j++)
         {
          __half temp=0;
             ...
             {
                //cout << "in cblas";
                //C_cblas[j * M + i] = (  alpha * A[k * M + i] * B[j * K + k] ) + beta * C_cblas[j * M + i]  ; //CM N,N
...
void func_check_hgemmNN_Col_type_1(int M, int N, int K, __half alpha, __half beta, float tolerance) {
  ...
  for(int i = 0; i < M * K; i++) {
   ...
   // A[i] = 1;//(unsigned short) ((static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X))) * 100);
  }

  for(int i = 0; i < K * N; i++) {
    B[i] = __hc_float2half (static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X)));
    //B[i] = 1;//(unsigned short) ((static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X))) * 100);
  }

  for(int i = 0; i < M * N; i++) {
    C[i] = __hc_float2half (static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X)));
    //C[i] = 1;//(unsigned short) ((static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X))) * 100);
    C_cblas[i] = C[i];
  }
  //cout << "while execution";
  accl_view.copy(A, devA, M * K * sizeof(__half));
  accl_view.copy(B, devB, K * N * sizeof(__half));
  accl_view.copy(C, devC, M * N * sizeof(__half));
  // NoTransA and NoTransB
  ...
  /*__half result = hgemmCompareL2fe(C_cblas, C_hcblas, M*N, tolerance);
  EXPECT_LE(result, tolerance);*/
  cout << "here";
  for(int i = 0 ; i < M * N ; i++)
  {
    //cout <<endl<<C_cblas[i]<<"\t"<<C_hcblas[i] ;
...
void func_check_hgemmNT_Col_type_1(int M, int N, int K, __half alpha, __half beta, float tolerance) {
  ...
  float X = 2;
  //__half Z ;
  //Z = __hc_float2half (static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X)));
  for(int i = 0; i < M * K; i++) {
    A[i] = __hc_float2half (static_cast <float> (rand()) / (static_cast <float> (RAND_MAX/X)));
  }
  ...
  for(int i = 0 ; i < M * N ; i++)
  {
    if(C_hcblas[i] != C_cblas[i] )
    cout << endl<<i<<"\t"<<C_hcblas[i]<<"\t"<< C_cblas[i]<<"\t"<<A[i]<<"\t"<<B[i]<<endl;
    EXPECT_EQ(C_hcblas[i], C_cblas[i]);
  }
#if 0
  for(int i = 0 ; i < M * N ; i++) {
...
/*TEST(hcblas_hgemm, func_correct_hgemmNN_Col_square_vvsmall_Implementation_type_1) {
 ...
 M = N = K = gen_vvsmall();
 //cout<<M<<N<<K; 
 //cout <<"\nbefore variable assigning";
 //cout << __hc_float2half((float)rand()/(float)(RAND_MAX) * 1.172) ;
...
/*TEST(hcblas_hgemm, func_correct_hgemmTN_Col_square_large_Implementation_type_1) {
 ...
 M = N = K = gen_large();
 __half alpha = __hc_float2half((float)rand()/(float)(RAND_MAX) * 1.172);
 __half beta = ((__half)rand()/(__half)(RAND_MAX)) * 3.414; 
 func_check_hgemmTN_Col_type_1(M, N, K, alpha, beta, 1.0e-5f);
} */
...
TEST(hcblas_hgemm, func_correct_hgemmNT_Col_slimC_smallN_Implementation_type_1) {
 ...
 M = K = gen_vlarge();
 N = gen_small();
 __half alpha = ((__half)rand()/(__half)(RAND_MAX)) * 1.172; 
 __half beta = ((__half)rand()/(__half)(RAND_MAX)) * 3.414; 
 func_check_hgemmNT_Col_type_1(M, N, K, alpha, beta, 1.0e-5f);
}
...
/*TEST(hcblas_hgemm, return_correct_hgemm_Implementation_type_2) {
   hc::accelerator accl;
   Hcblaslibrary hc(&accl);
  int M = 189, N = 9, K = 19;
  __half alpha = 1, beta = 1;
  ...
  accelerator_view accl_view = hc.currentAcclView;
  accelerator acc = hc.currentAccl;
  // Implementation type II - Inputs and Outputs are HCC __half array containers with batch processing
  __half* Abatch = (__half*) calloc(M * K, sizeof(__half));
  __half* Bbatch = (__half*) calloc(K * N, sizeof(__half));
  __half* Cbatch = (__half*) calloc(M * N * batchSize, sizeof(__half));
  __half* devAbatch = hc::am_alloc(sizeof(__half) * M * K, acc, 0);
  __half* devBbatch = hc::am_alloc(sizeof(__half) * K * N, acc, 0);
  __half* devCbatch = hc::am_alloc(sizeof(__half) * M * N * batchSize, acc, 0);
  ...
  accl_view.copy(Abatch, devAbatch, M * K * sizeof(__half));
  accl_view.copy(Bbatch, devBbatch, K * N * sizeof(__half));
  accl_view.copy(Cbatch, devCbatch, M * N * batchSize * sizeof(__half));
  // NoTransA and NoTransB
  typeA = NoTrans;
  typeB = NoTrans;
  // Column major
  ...
  ldb = K ;
  ldc = M;
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  // Row Major
  lda = K;
  ldb = N ;
  ldc = N;
  status = hc.hcblas_hgemm(accl_view, RowMajor, typeA, typeB, M, N, K, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  // NoTransA TransB
  typeA = NoTrans;
  typeB = Trans;
  ...
  __half* devC1 = NULL;
  // A, B, C device pointers are not allocated properly 
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, alpha, devA1, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, alpha, devAbatch, lda, A_batchOffset, devB1, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devC1, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  // M is 0
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, 0, N, K, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  // N is 0
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, 0, K, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  // K is 0
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, 0, alpha, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_INVALID);
  free(Abatch);
  free(Bbatch);
  free(Cbatch);
...
/* prasanth
TEST(hcblas_hgemm, func_correct_hgemm_Implementation_type_2) {
   hc::accelerator accl;
   Hcblaslibrary hc(&accl);
  int M = 189, N = 9, K = 19;
  __half alpha = 1, beta = 1;
  ...
  __half *B[batchSize];
  __half *C[batchSize];
  __half *C_hcblas[batchSize];
  __half *C_cblas[batchSize];
  // Device pointers stored in host memory
  __half *devA[batchSize], *devB[batchSize], *devC[batchSize];
  // Create device double pointer to store device pointers in device memory
  __half **d_Aarray =  hc::am_alloc(batchSize * sizeof(__half*), acc, 0);
  __half **d_Barray  = hc::am_alloc(batchSize * sizeof(__half*), acc, 0);
  __half **d_Carray = hc::am_alloc(batchSize * sizeof(__half*), acc, 0);
  const size_t aSize = sizeof(__half) * M * K;
  const size_t bSize = sizeof(__half) * K * N;
  const size_t cSize = sizeof(__half) * M * N;

  // Host and Device Array allocation
  for(int i =0; i < batchSize; i++) {
    A[i] = (__half *) malloc(aSize);
    B[i] = (__half *) malloc(bSize);
    C[i] = (__half *) malloc(cSize);
    C_hcblas[i] = (__half *) malloc(cSize);
    C_cblas[i] = (__half *) malloc(cSize);
  ...
  for (int b = 0; b < batchSize; b++) {
    ...
    accl_view.copy(A[b], devA[b], sizeof(__half) * M * K);
    for(int i = 0; i < K * N; i++) {
      B[b][i] = rand() % 15;
    }
    accl_view.copy(B[b], devB[b], sizeof(__half) * K * N);
    for(int i = 0; i < M * N;i++) 
    {
      C[b][i] = 4;
      C_cblas[b][i] = C[b][i];
    }
    accl_view.copy(C[b], devC[b], sizeof(__half) * M * N);
  } prasanth*/

  // NoTransA and NoTransB */           
/* prasanth  typeA = NoTrans;
  typeB = NoTrans;
  Transa = CblasNoTrans;
  Transb = CblasNoTrans;

  // Copyinng device pointers stored in host memory to device memory
  accl_view.copy(devA, d_Aarray, batchSize * sizeof(__half*));
  accl_view.copy(devB, d_Barray, batchSize * sizeof(__half*));
  accl_view.copy(devC, d_Carray, batchSize * sizeof(__half*));

  // Column major
  lda = M;
  ldb = K ;
  ldc = M;
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, alpha, d_Aarray, lda, A_batchOffset, d_Barray, ldb, B_batchOffset, beta, d_Carray, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
// Get the results
  for(int b =0; b < batchSize; b++) {
    accl_view.copy(devC[b], C_hcblas[b], M * N * sizeof(__half));
  }

  for(int b = 0; b < batchSize; b++)
         cblas_hgemm(CblasColMajor, Transa, Transb, M, N, K, alpha, A[b], lda, B[b], ldb, beta, C_cblas[b],ldc );
  ...
  hc::am_free(d_Aarray);
  hc::am_free(d_Barray);
  hc::am_free(d_Carray); prasanth */
/*  // alpha = 0
  lda = M;
  ldb = K ;
  ldc = M;
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, 0, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  accl_view.copy(devCbatch, Chcblasbatch, M * N * batchSize * sizeof(__half));

  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasColMajor, Transa, Transb, M, N, K, 0, Abatch, lda, Bbatch, ldb, beta, CCblasbatch  + i * M * N , ldc );
  }
  ...
  ldb = K ;
  ldc = M;
  status = hc.hcblas_hgemm(accl_view, ColMajor, typeA, typeB, M, N, K, 0, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, 0, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  accl_view.copy(devCbatch, Chcblasbatch, M * N * batchSize * sizeof(__half));

  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasColMajor, Transa, Transb, M, N, K, 0, Abatch, lda, Bbatch, ldb, 0, CCblasbatch  + i * M * N , ldc );
  }
  ...
  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasRowMajor, Transa, Transb, M, N, K, alpha, Abatch, lda, Bbatch, ldb, beta, CCblasbatch  + i * M * N , ldc );
  }
  ...
  ldb = N ;
  ldc = N;
  status = hc.hcblas_hgemm(accl_view, RowMajor, typeA, typeB, M, N, K, 0, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, beta, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  accl_view.copy(devCbatch, Chcblasbatch,  M * N * batchSize * sizeof(__half));

  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasRowMajor, Transa, Transb, M, N, K, 0, Abatch, lda, Bbatch, ldb, beta, CCblasbatch  + i * M * N , ldc );
  }
  ...
  ldb = N ;
  ldc = N;
  status = hc.hcblas_hgemm(accl_view, RowMajor, typeA, typeB, M, N, K, 0, devAbatch, lda, A_batchOffset, devBbatch, ldb, B_batchOffset, 0, devCbatch, ldc, C_batchOffset, aOffset, bOffset, cOffset, batchSize);
  EXPECT_EQ(status, HCBLAS_SUCCEEDS);
  accl_view.copy(devCbatch, Chcblasbatch, M * N * batchSize * sizeof(__half));

  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasRowMajor, Transa, Transb, M, N, K, 0, Abatch, lda, Bbatch, ldb, 0, CCblasbatch  + i * M * N , ldc );
  }
  ...
  for(int i = 0; i < batchSize; i++) {
    cblas_hgemm( CblasColMajor, Transa, Transb, M, N, K, alpha, Abatch, lda, Bbatch, ldb, beta, CCblasbatch  + i * M * N , ldc );
  }
