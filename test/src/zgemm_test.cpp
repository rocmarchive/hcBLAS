...
/* Implementation type I - Inputs and Outputs are HCC double array containers */
    ...
    if(Imple_type == 1) {
        double_2* a = (double_2 *)malloc(sizeof(double_2 )* M * K);
        double_2* b = (double_2 *)malloc(sizeof(double_2 )* K * N);
        double_2* c = (double_2 *)malloc(sizeof(double_2 )* M * N);
    	...
    	status = hc.hcblas_zgemm(accl_view, hcOrder, typeA, typeB, M, N, K, cAlpha, devA, aOffset, lda, devB, bOffset, ldb, cBeta, devC, cOffset, ldc);
...
/* Implementation type II - Inputs and Outputs are HCC double array containers with batch processing */
        ...
        double_2 **devAbatch = hc::am_alloc(sizeof(double_2*) * batchSize, acc[1], 0);
        double_2 **devBbatch = hc::am_alloc(sizeof(double_2*) * batchSize, acc[1], 0);
        double_2 **devCbatch = hc::am_alloc(sizeof(double_2*) * batchSize, acc[1], 0);
        ...
        { 
           Abatch[i] = (double_2*) calloc(M * K, sizeof(double_2));
           Bbatch[i] = (double_2*) calloc(K * N, sizeof(double_2));
           Cbatch[i] = (double_2*) calloc(M * N, sizeof(double_2));
           devAbatch[i] = hc::am_alloc(sizeof(double_2) * M * K, acc[1], 0);
           devBbatch[i] = hc::am_alloc(sizeof(double_2) * K * N, acc[1], 0);
           devCbatch[i] = hc::am_alloc(sizeof(double_2) * M * N, acc[1], 0);
           abatch[i] = (double *)malloc(sizeof(double )* M * K * 2);
           bbatch[i] = (double *)malloc(sizeof(double )* K * N * 2);
           cbatch[i] = (double *)malloc(sizeof(double )* M * N * 2);
        }
        ...
        accl_view.copy(Abatch, devAbatch, M * K * batchSize * sizeof(double_2));
        accl_view.copy(Bbatch, devBbatch, K * N * batchSize * sizeof(double_2));
        accl_view.copy(Cbatch, devCbatch, M * N * batchSize * sizeof(double_2));
    	status = hc.hcblas_zgemm(accl_view, hcOrder, typeA, typeB, M, N, K, cAlpha, devAbatch, aOffset, A_batchOffset, lda, devBbatch, bOffset, B_batchOffset, ldb, cBeta, devCbatch, cOffset, C_batchOffset, ldc, batchSize);
        accl_view.copy(devCbatch, Cbatch,  M * N * batchSize * sizeof(double_2)); 
        for(int i = 0; i < batchSize;i++)
	     cblas_zgemm( order, Transa, Transb, M, N, K, &alpha, abatch + i * M * K * 2, lda, bbatch + i * K * N * 2, ldb, &beta, cbatch + i * M * N * 2, ldc );
