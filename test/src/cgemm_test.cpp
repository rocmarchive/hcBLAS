    ...
    for(int iter = 0; iter < 10; iter++) {
      ...
      for(int i = 0, k=0; ((i < M * N) && ( k < M * N * 2)) ; i++, k=k+2) { 
         if( cblas[k] != (C[i].x) || cblas[k+1] != (C[i].y) ) {
           ispassed = 0;
           cout << " HCSGEMM["<<i<<"].x = "<<C[i].x<<" doesnot match with CBLASSGEMM["<<i<<"] =" << cblas[k];
           cout << " HCSGEMM["<<i<<"].y = "<<C[i].y<<" doesnot match with CBLASSGEMM["<<i<<"] =" << cblas[k+1]<< endl;
           break;
         }
         else
           continue;
...
/* Implementation type II - Inputs and Outputs are HCC float array containers with batch processing */
    ...
    for(int i = 0; i < batchSize; i++)
      cblas_cgemm( order, Transa, Transb, M, N, K, alpha, ablas[i], lda, bblas[i], ldb, beta, cblas[i] ,ldc );

    for(int b = 0; b < batchSize; b++){
      for(int i = 0, k=0 ; ((i < M * N) && ( k < M * N * 2)) ; i++, k=k+2){ 
        if( C[b][i].x != cblas[b][k] || C[b][i].y != cblas[b][k+1] ){
          ispassed = 0;
          cout << " HCCGEMM["<<b<<"]["<<i<<"].x = "<<C[b][i].x<<" doesnot match with CBLASCGEMM["<<b<<"]["<<k<<"] =" << cblas[b][k];
          cout << " HCCGEMM["<<b<<"]["<<i<<"].y = "<<C[b][i].y<<" doesnot match with CBLASCGEMM["<<b<<"]["<<(k+1)<<"] =" << cblas[b][k+1] << endl;
          break;
        }
        else
          continue;
