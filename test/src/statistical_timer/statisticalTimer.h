...
class StatisticalTimer {
  ...
  //	In order to calculate statistics <std. dev.>, we need to keep a history
  // of our timings
  ...
  //	This setting controls whether the Timer should convert samples into time
  // by dividing by the
  //	clock frequency
  bool normalize;
   ...
   *	This is private so that user code cannot create their own instantiation.
   *Instead, you
   ...
   * \brief Assignment operator does not make sense for a singleton, disallow
   * assignments
   */
 ...
 public:
  ...
  //	Variance - average of the squared differences between data points and
  // the mean
  ...
  double gflops(double Time, int M, int N, int K) const;
  /**
  ...
  //	Using the stdDev of the entire population (of an id), eliminate those
  // samples that fall
  //	outside some specified multiple of the stdDev.  This assumes that the
  // population
