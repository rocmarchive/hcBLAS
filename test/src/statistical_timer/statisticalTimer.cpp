...
//	Currently, RangeType is expected to be a floating point type, and
//ValType an integer type
template <typename RangeType, typename ValType>
struct PruneRange {
  RangeType lower, upper;

  PruneRange(RangeType mean, RangeType stdev)
  ...
  bool operator()(ValType val) {
    ...
    //	This is why we cast ValType to RangeType, because RangeType should
    //always be floating and signed
...
StatisticalTimer& StatisticalTimer::getInstance() {
  static StatisticalTimer timer;
  return timer;
}

StatisticalTimer::StatisticalTimer()
    : nEvents(0), nSamples(0), normalize(true) {
#if defined(_WIN32)
...
void StatisticalTimer::Reset() {
  if (nEvents == 0 || nSamples == 0)
    throw std::runtime_error(
        "StatisticalTimer::Reserve( ) was not called before Reset( )");
...
//	nEvents is an approximate value for how many seperate events the caller
//will think
//	they will need, and nSamples is a hint on how many samples we think we
//will take
//	per event
void StatisticalTimer::Reserve(unsigned int nEvents, unsigned int nSamples) {
...
void StatisticalTimer::setNormalize(bool norm) { normalize = norm; }

void StatisticalTimer::Start(sTimerID id) {
#if defined(_WIN32)
  ...
  gettimeofday(&s, 0);
  clkStart.at(id) =
      (unsigned long long)s.tv_sec * 1000000 + (unsigned long long)s.tv_usec;
#endif
}

void StatisticalTimer::Stop(sTimerID id) {
...
void StatisticalTimer::AddSample(const sTimerID id,
                                 const unsigned long long n) {
  clkTicks.at(id).push_back(n);
}

//	This function's purpose is to provide a mapping from a 'friendly' human
//readable text string
//	to an index into internal data structures.
StatisticalTimer::sTimerID StatisticalTimer::getUniqueID(
    const std::string& label, unsigned int groupID) {
  ...
  if (iter != labelID.end()) return std::distance(labelID.begin(), iter);
...
double StatisticalTimer::getMean(sTimerID id) const {
  if (clkTicks.empty()) return 0;

  size_t N = clkTicks.at(id).size();

  Accumulator<unsigned long long> sum =
      std::for_each(clkTicks.at(id).begin(), clkTicks.at(id).end(),
                    Accumulator<unsigned long long>());

  return static_cast<double>(sum.acc) / N;
}

double StatisticalTimer::getVariance(sTimerID id) const {
  if (clkTicks.empty()) return 0;
...
double StatisticalTimer::getStdDev(sTimerID id) const {
  double variance = getVariance(id);

  return sqrt(variance);
}

double StatisticalTimer::getAverageTime(sTimerID id) const {
  if (normalize)
...
double StatisticalTimer::getMinimumTime(sTimerID id) const {
  clkVector::const_iterator iter =
      std::min_element(clkTicks.at(id).begin(), clkTicks.at(id).end());
...
unsigned int StatisticalTimer::pruneOutliers(sTimerID id, double multiple) {
  if (clkTicks.empty()) return 0;
  ...
  //	std::remove_if does not actually erase, it only copies elements, it
  //returns new 'logical' end
  clkVector::iterator newEnd = std::remove_if(
      clks.begin(), clks.end(),
      PruneRange<double, unsigned long long>(mean, multiple * stdDev));

  clkVector::difference_type dist = std::distance(newEnd, clks.end());

  if (dist != 0) clks.erase(newEnd, clks.end());
...
unsigned int StatisticalTimer::pruneOutliers(double multiple) {
  unsigned int tCount = 0;

  for (unsigned int l = 0; l < labelID.size(); ++l) {
    unsigned int lCount = pruneOutliers(l, multiple);
    std::clog << "\tStatisticalTimer:: Pruning " << lCount << " samples from "
              << labelID[l].first << std::endl;
    tCount += lCount;
  }
...
double StatisticalTimer::gflops(double Time, int M, int N, int K) const {
  return ((2.0 * M * N * K) / Time);
}

//	Defining an output print operator
std::ostream& operator<<(std::ostream& os, const StatisticalTimer& st) {
  if (st.clkTicks.empty()) return os;

  std::ios::fmtflags bckup = os.flags();

  for (unsigned int l = 0; l < st.labelID.size(); ++l) {
    unsigned long long min = 0;
    StatisticalTimer::clkVector::const_iterator iter =
        std::min_element(st.clkTicks.at(l).begin(), st.clkTicks.at(l).end());

    if (iter != st.clkTicks.at(l).end()) min = *iter;

    os << st.labelID[l].first << ", " << st.labelID[l].second << std::fixed
       << std::endl;
    os << "Min:," << min << std::endl;
