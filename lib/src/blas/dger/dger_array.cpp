...
void ger_HC(hc::accelerator_view accl_view,
  ...
  hc::parallel_for_each(accl_view, compute_domain.tile(16, 16), [ = ] (hc::tiled_index<2>& tidx) [[hc]] {
    int i = tidx.global[0];
    int j = tidx.global[1];
...
void ger_HC(hc::accelerator_view accl_view,
  ...
  hc::parallel_for_each(accl_view, compute_domain.tile(1, 16, 16), [ = ] (hc::tiled_index<3>& tidx) [[hc]] {
    int elt = tidx.tile[0];
    int i = tidx.global[1];
    int j = tidx.global[2];
