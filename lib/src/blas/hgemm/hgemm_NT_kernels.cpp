...
hcblasStatus gemm_NoTransA_MICRO_NBK_Mini_Batch_M128_N128_K16_TS16XMTS2_MB2(hc::accelerator_view accl_view,
  ...
  hc::parallel_for_each(accl_view, t_ext, [ = ] (hc::tiled_index<2> tidx) __attribute__((hc, cpu)) {
    ...
    do {
      ...
      lB[lIndex + 0 * 16 * 2 + 0] = B[BIndex + block_k * 16 * ldb + 0 * 16 * 2 + 0];
      lB[lIndex + 0 * 16 * 2 + 1] = B[BIndex + block_k * 16 * ldb + 0 * 16 * 2 + 1];
      lB[lIndex + 1 * 16 * 2 + 0] = B[BIndex + block_k * 16 * ldb + 1 * 16 * 2 + 0];
      lB[lIndex + 1 * 16 * 2 + 1] = B[BIndex + block_k * 16 * ldb + 1 * 16 * 2 + 1];
      lA[lIndex + 0 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 0 * 16 * 2 + 0];
      lA[lIndex + 0 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 0 * 16 * 2 + 1];
      lA[lIndex + 1 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 1 * 16 * 2 + 0];
      lA[lIndex + 1 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 1 * 16 * 2 + 1];
...
hcblasStatus gemm_NoTransA_MICRO_NBK_Mini_Batch_M128_N128_K16_TS16XMTS4_MB2(hc::accelerator_view accl_view,
  ...
  hc::parallel_for_each(accl_view, t_ext, [ = ] (hc::tiled_index<2>& tidx) __attribute__((hc, cpu)) {
    ...
    do {
      ...
      lB[lIndex + 1 * 16 * 2 + 0] = B[BIndex + block_k * 16 * ldb + 1 * 16 * 2 + 0];
      lB[lIndex + 1 * 16 * 2 + 1] = B[BIndex + block_k * 16 * ldb + 1 * 16 * 2 + 1];
      lB[lIndex + 2 * 16 * 2 + 0] = B[BIndex + block_k * 16 * ldb + 2 * 16 * 2 + 0];
      lB[lIndex + 2 * 16 * 2 + 1] = B[BIndex + block_k * 16 * ldb + 2 * 16 * 2 + 1];
      lB[lIndex + 3 * 16 * 2 + 0] = B[BIndex + block_k * 16 * ldb + 3 * 16 * 2 + 0];
      lB[lIndex + 3 * 16 * 2 + 1] = B[BIndex + block_k * 16 * ldb + 3 * 16 * 2 + 1];
      lA[lIndex + 0 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 0 * 16 * 2 + 0];
      lA[lIndex + 0 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 0 * 16 * 2 + 1];
      lA[lIndex + 1 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 1 * 16 * 2 + 0];
      lA[lIndex + 1 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 1 * 16 * 2 + 1];
      lA[lIndex + 2 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 2 * 16 * 2 + 0];
      lA[lIndex + 2 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 2 * 16 * 2 + 1];
      lA[lIndex + 3 * 16 * 2 + 0] = A[AIndex + block_k * 16 * lda + 3 * 16 * 2 + 0];
      lA[lIndex + 3 * 16 * 2 + 1] = A[AIndex + block_k * 16 * lda + 3 * 16 * 2 + 1];
    ...
    do {
      ...
      if(gidx == M_block-1 || gidy == N_block-1 || block_k == K_block-1)
      {
          for(int sec = 0; sec < 2; ++sec) {
            ...
            if( (gidy*32*2 + idx * 2 + secVal * 2 + 0) < N && (block_k * 16 + idy) < K) {
              lB[ blIndex + secVal * 2 + 0] = B[BIndex + BinitOffset + secVal * 2 + 0];
            } else {
              lB[blIndex + secVal * 2 + 0] = 0;
            }
            if( (gidy*32*2 + idx * 2 + secVal * 2 + 1) < N && (block_k * 16 + idy) < K) {
              lB[ blIndex + secVal * 2 + 1] = B[BIndex + BinitOffset + secVal * 2 + 1];
            } else {
    ...
    do {
      ...
      if(gidx == M_block-1 || gidy == N_block-1 || block_k == K_block-1)
      {
          for(int sec = 0; sec < 4; ++sec) {
            ...
            if( (gidy*64*2 + idx * 2 + secVal * 2 + 0) < N && (block_k * 16 + idy) < K) {
              lB[ blIndex + secVal * 2 + 0] = B[BIndex + BinitOffset + secVal * 2 + 0];
            } else {
              lB[blIndex + secVal * 2 + 0] = 0;
            }
            if( (gidy*64*2 + idx * 2 + secVal * 2 + 1) < N && (block_k * 16 + idy) < K) {
              lB[ blIndex + secVal * 2 + 1] = B[BIndex + BinitOffset + secVal * 2 + 1];
            } else {
              lB[blIndex + secVal * 2 + 1] = 0;
            }
...
#define MICROTILESIZE 6
  ...
  hc::parallel_for_each(accl_view, t_ext, [ = ] (hc::tiled_index<2>& tidx) __attribute__((hc, cpu)) {
    ...
    do {
      ...
      lB[lIndex + 2 * TILESIZE] = B[BIndex + block_k * TILESIZE * ldb + 2 * TILESIZE];
      lB[lIndex + 3 * TILESIZE] = B[BIndex + block_k * TILESIZE * ldb + 3 * TILESIZE];
      lB[lIndex + 4 * TILESIZE] = B[BIndex + block_k * TILESIZE * ldb + 4 * TILESIZE];
      lB[lIndex + 5 * TILESIZE] = B[BIndex + block_k * TILESIZE * ldb + 5 * TILESIZE];
      lA[lIndex + 0 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 0 * TILESIZE];
      lA[lIndex + 1 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 1 * TILESIZE];
      lA[lIndex + 2 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 2 * TILESIZE];
      lA[lIndex + 3 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 3 * TILESIZE];
      lA[lIndex + 4 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 4 * TILESIZE];
      lA[lIndex + 5 * TILESIZE] = A[AIndex + block_k * TILESIZE * lda + 5 * TILESIZE];
