...
int AutogemmKernel::makeGemmKernel(AutogemmKernel* gemmKernel, kernTypes* kernelType, std::string& kStr) {
  ...
  if (!isFourKernel) {
    ...
    kStr = kStr + "#define TILE_SIZE        " + to_string(gemmKernel->tileNumRows) + endLine;
    kStr = kStr + "#define MICRO_TILE_SIZE  " + to_string(gemmKernel->microtileNumRows) + endLine;
    kStr = kStr + "#define MACRO_TILE_SIZE  " + to_string(gemmKernel->tileNumRows*gemmKernel->microtileNumCols) + endLine;
    ...
    kStr = kStr +
      ...
      "  DATA_TYPE_STR rC[MICRO_TILE_SIZE][MICRO_TILE_SIZE] = { {0} };" + endLine +
      "  DATA_TYPE_STR rA[MICRO_TILE_SIZE];" + endLine +
      "  DATA_TYPE_STR rB[MICRO_TILE_SIZE];" + endLine;
   ...
   kStr = kStr + endLine +
      ...
      "  uint alIndex = (idy * (TILE_SIZE * MICRO_TILE_SIZE + 1) + idx);" + endLine +
      "  uint blIndex = (idx * (TILE_SIZE * MICRO_TILE_SIZE + 1) + idy);" + endLine +
      "  uint AIndex = offsetA + (gidx * TILE_SIZE * MICRO_TILE_SIZE) + idx + (idy * lda);" + endLine +
      "  uint BIndex = offsetB + ((gidy * TILE_SIZE * MICRO_TILE_SIZE) + idy) * ldb + idx;" + endLine +
    ...
    int numALoads = gemmKernel->microtileNumRows;
    int numBLoads = gemmKernel->microtileNumCols;

    kStr = kStr +
           "    if(gidx == M_block-1 || gidy == N_block-1 || block_k == K_block-1) {" + endLine +
           "      for(int sec = 0; sec < MICRO_TILE_SIZE; ++sec) {" + endLine +
           "        int secVal = sec * TILE_SIZE;" + endLine + endLine +
           "        if( (gidy * MACRO_TILE_SIZE + idy + secVal) < N && (block_k * TILE_SIZE + idx) < K) {" + endLine +
           "          lB[blIndex + secVal] = B[BIndex + BinitOffset + secVal * ldb];" + endLine + 
           "        } else {" + endLine +
           "          lB[blIndex + secVal] = 0;" + endLine "          }" + endLine + endLine +
           "        if( (gidx * MACRO_TILE_SIZE + idx + secVal) < M && (block_k * TILE_SIZE + idy) < K) {" + endLine +
           "          lA[alIndex + secVal] = A[AIndex + secVal + AinitOffset];" + endLine +
           "        } else {" + endLine + 
           "          lA[alIndex + secVal] = 0;" + endLine + "          }" + endLine +
           "      }" + endLine +
           "    } else {" + endLine;

    for ( int a = 0; a < numALoads; a++) 
    kStr = kStr + 
           "      lB[blIndex + TILE_SIZE * " + to_string(a) + " ] =  B[BIndex + BinitOffset + TILE_SIZE * " + to_string(a) + "* ldb];" + endLine;

    for ( int a = 0; a < numALoads; a++) 
    kStr = kStr + 
           "      lA[alIndex + TILE_SIZE * " + to_string(a) + " ] =  A[AIndex + AinitOffset + TILE_SIZE * " + to_string(a) + "];" + endLine;
    ...
    kStr = kStr + "  /* write global Cij */" + endLine +
                + "  if(gidx == M_block-1 || gidy == N_block-1) { " + endLine;
   ...
   // end parallel for each
    ...
    // kernel parameters
    kStr += endLine;
    kStr = kStr + "#undef TILE_SIZE          " + endLine;
    kStr = kStr + "#undef MICRO_TILE_SIZE  " + endLine;
    kStr = kStr + "#undef MACRO_TILE_SIZE  " + endLine;
